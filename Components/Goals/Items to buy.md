...
if (pathParts[0] === "Goals" && pathParts[1] && pathParts[2]) {
    ...
    const folder = `${goalFolder}/${category === "Wishlist" ? "Items" : "Budget"}`;
    ...
    let items = dv.pages(`"${folder}"`)
        .sort(item => Number(item.price) || 0, "desc");

    // Bought Wishlist items should not appear in the shopping list
    if (category === "Wishlist") {
        items = items.where(item => !isBought(item));
    }
    ...
    for (const item of items) {
        ...
        checkbox.checked = isBought(item);
